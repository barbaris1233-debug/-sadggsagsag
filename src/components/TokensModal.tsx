...
export default function TokensModal({ open, onClose }: Props) {
  ...
  const handleSave = useCallback(() => {
    setBotTokens(draft.filter(isValid));
    onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#0d0d0d] border border-white/10 rounded-2xl w-[480px] max-h-[85vh] flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-500">Каждый токен = +1 параллельный поток</p>
            </div>
          </div>
        ...
        <div className="mx-5 mt-4 px-4 py-2.5 rounded-xl bg-violet-500/5 border border-violet-500/10 flex items-center gap-2">
          ...
          <span className="text-[11px] text-gray-400">
            {draft.length === 0
              ? <span>Без токенов: proxy-режим</span>
              : <><span className="text-violet-400 font-semibold">{draft.length}</span> токен{draft.length > 1 ? 'а' : ''} → каналы/группы/боты проверяются мгновенно</>
            }
          </span>
        </div>

        <div className="px-5 py-3 space-y-1.5 max-h-64 overflow-y-auto">
          {draft.length === 0 && <p className="text-xs text-gray-600 text-center py-4">Токены не добавлены — работает proxy-режим</p>}
          {draft.map((token, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06] group">
                {isValid(token) ? <CheckCircle size={13} className="text-emerald-400 shrink-0" /> : <AlertCircle size={13} className="text-red-400 shrink-0" />}
                <span className="flex-1 text-xs font-mono text-gray-400 truncate">{masked}</span>
                <button onClick={() => setDraft((d) => d.filter((_, j) => j !== i))} className="text-gray-600 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 cursor-pointer">
                  <Trash2 size={13} />
                </button>
              </div>
            );
        ...
        <div className="px-5 pb-3 space-y-1.5">
          <div className="flex gap-2">
            <input
              ...
              spellCheck={false}
            />
            <button onClick={handleAdd} disabled={!input.trim()} className="px-3 py-2 rounded-xl bg-violet-500/10 text-violet-400 border border-violet-500/20 hover:bg-violet-500/20 transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer">
              <Plus size={15} />
            </button>
          </div>
        ...
        <div className="px-5 pb-4 pt-3 border-t border-white/5 flex items-center justify-between">
          <a href="https://t.me/BotFather" target="_blank" rel="noopener noreferrer" className="text-[11px] text-gray-600 hover:text-gray-400 transition-colors">
            Создать токен → @BotFather
          </a>
          <div className="flex gap-2">
            <button onClick={onClose} className="px-4 py-2 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-white/5 transition-colors cursor-pointer">Отмена</button>
            <button onClick={handleSave} className="px-4 py-2 rounded-lg text-xs font-medium bg-violet-500/20 text-violet-400 border border-violet-500/30 hover:bg-violet-500/30 transition-colors cursor-pointer">Сохранить</button>
          </div>
        </div>
      </div>
    </div>
