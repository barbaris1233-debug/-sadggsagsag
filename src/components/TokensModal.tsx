      ...
      <div className="bg-[#0d0d0d] border border-white/10 rounded-2xl w-[480px] max-h-[85vh] flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mx-5 mt-4 px-4 py-2.5 rounded-xl bg-violet-500/5 border border-violet-500/10 flex items-center gap-2">
          ...
          <span className="text-[11px] text-gray-400">
            {draft.length === 0
              ? <span>Без токенов: proxy-режим, <span className="text-white">3</span> параллельно</span>
              : <>
                  <span className="text-violet-400 font-semibold">{draft.length}</span> токен{draft.length > 1 ? 'ов' : ''}{' '}→{' '}
                  <span className="text-emerald-400 font-semibold">{draft.length * 4}</span> параллельных проверок
                </>
            }
          </span>
        </div>
        ...
        <div className="px-5 pb-4 pt-3 border-t border-white/5 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSave} className="px-4 py-2 rounded-lg text-xs font-medium bg-violet-500/20 text-violet-400 border border-violet-500/30 hover:bg-violet-500/30 transition-colors cursor-pointer">
              Сохранить
            </button>
          </div>
        </div>
